...
| rules | RULES.md 恒真规则管理器：`/rules show` `/rules init` `/rules reload`；`@import` 精细导入（单文件 / `#标题` / glob / 递归 5 层 / 防环去重），`@import-if` 条件导入（os: 平台 / env: 变量 / env: 变量=值，`!` 取反，不满足静默跳过记为 [skip]），`@rules` 可配置参数，`&lt;!-- --&gt;` 注释剥离，结果注入 system prompt；TUI 下 `show` 以可折叠规则树展示（↑↓ 选择，← 折叠 / → 展开 / Enter 切换，叶子为具体规则行，imports / Settings / Diagnostics 默认折叠） |
