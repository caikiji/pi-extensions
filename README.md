...
| checkpoint | git 存档点：`/checkpoint <msg>` 快照、`/restore <id|latest>` 只还原快照涉及文件（不碰无关改动）、`drop` 清理；冲突保护（你之后改过的文件需 `--force`）；每轮对话自动存档（保留最近 20 个），`checkpoint_list` / `checkpoint_restore` 工具供 agent 调用 |
