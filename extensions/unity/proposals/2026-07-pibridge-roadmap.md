...
**注意：限流与合并。** `error_occurred` 可能高频触发（如每帧报错）。实现时需做两层防护：
① PiBridge 侧按**事件类型 + 强标识**（如错误指纹：异常类型+文件+行号）去重，同类事件至少间隔几秒。
② extension 侧限流，Agent 忙碌时 `sendUserMessage` 自然排队，不清醒时合并为一条摘要。
