...
- 提交的代码与配置文件不得含机器特定的绝对路径（用户目录、nvm、npm 全局安装目录、盘符等）；类型检查所需依赖在 package.json 的 devDependencies / peerDependencies 声明，`npm install` 即可（node_modules 不提交），无需任何机器相关配置——由 `tests/portable.test.mjs` 机械检查

## 意图 (layout changes, intent does not)

- 保持"clone 即跑"：无构建步骤、任何机器可测（`npm test` 不需要 node_modules；npm install 仅用于本地 IDE 类型提示）
- 每个扩展解决一个具体痛点，不过度设计
- 本仓库是个人工具，但代码质量按开源标准
